...
const PokedexBox: React.FC<Props> = ({ selectedPokemon, handlePokedexClose, loading }) => {
    ...
    const renderAbilities = (abilities?: Ability[]) => {
        return abilities?.map((ability, index) => (
            <p key={index} className={styles.skill}>{index + 1} - {ability.ability.name}</p>
        ));
    };
    const renderStats = (stats?: Stats[]) => {
        return stats?.map((stat, index) => (
            <p key={index}>{stat.stat.name} <span>{stat.base_stat}</span></p>
        ));
    };
    ...
    return (
        <div className={styles.pokedex}>
            <div className={styles.pokeCard}>
                {selectedPokemon && (
                    <PokeCard loading={loading} name={selectedPokemon.name} imageUrl={selectedPokemon.sprites.other['official-artwork'].front_default}/>
                )}
            </div>
            ...
            <div className={styles.pokedexId}>
                {selectedPokemon && (
                    <p><span>{selectedPokemon.id}</span></p>
                )}
            </div>
            <div className={styles.pokeStats}>
                {renderStats(selectedPokemon?.stats)}
            </div>
